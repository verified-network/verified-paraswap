...
async function checkOnChainPricing(
  ...
  // Normally you can get it from verified.Iface or from eventPool.
  ...
  const readerResult = (
    await verified.dexHelper.multiContract.methods
...
describe('Verified', function () {
  ...
  describe('Mainnet', () => {
    ...
    it('getTopPoolsForToken', async function () {
      ...
      const newVerified = new Verified(network, dexKey, dexHelper);
      if (newVerified.updatePoolState) {
        await newVerified.updatePoolState();
      }
      const poolLiquidity = await newVerified.getTopPoolsForToken(
        tokens[srcTokenSymbol].address,
        10,
      );
      console.log(`${srcTokenSymbol} Top Pools:`, poolLiquidity);

      if (!newVerified.hasConstantPriceLargeAmounts) {
        checkPoolsLiquidity(
          poolLiquidity,
